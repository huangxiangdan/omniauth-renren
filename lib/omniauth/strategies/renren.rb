...
module OmniAuth
  module Strategies
    class Renren < OmniAuth::Strategies::OAuth2
      ...
      def request_phase
        options[:scope] ||= 'publish_feed photo_upload send_request'
        super
      end
      ...
      def raw_info
        @raw_info ||= MultiJson.decode(Net::HTTP.post_form(URI.parse('http://api.renren.com/restserver.do'), signed_params.merge({:fields => "uid,name,tinyurl,headhurl,zidou,star,sex"})).body)[0]
        @raw_info
